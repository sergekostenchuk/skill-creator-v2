...
## behavioral-run-manifest.json

Created by `scripts/prepare_behavioral_evals.py`. Located at `<workspace>/behavioral-run-manifest.json`.

```json
{
  "skill_name": "skill-creator-v2",
  "runtime_mode": "behavioral_packet",
  "adapter": "manual-multi-agent",
  ...
  "runs": [
    {
      ...
      "executor_prompt": "/path/to/executor-prompt.md",
      "result_path": "/path/to/outputs/result.md",
...
**Fields:**
- `runtime_mode`: `behavioral_packet` until actual executor output is recorded.
- `adapter`: External runner used or expected, such as `manual-multi-agent`, `codex-cli`, `claude-cli`, or `manual-json`.
- `actual_outputs_required`: Must be `true` for behavioral evidence.
- `inline_fallback_counts_as_behavioral`: Must be `false`.
- `runs[]`: Prepared executor packets and expected output locations.

---

## run-record.json

Created by `prepare_behavioral_evals.py` and updated by `record_behavioral_result.py`. Located at `<eval>/<config>/run-N/run-record.json`.
...
Behavioral grading must fail closed when `actual_output_recorded` is not true or `outputs/result.md` is missing.

---

## behavioral-eval-report.json

Created by `scripts/grade_behavioral_evals.py`. Located at `<workspace>/behavioral-eval-report.json`.
...
This report is behavioral evidence only when it grades actual recorded executor outputs. Prepared packets and inline fallback results are not enough.
